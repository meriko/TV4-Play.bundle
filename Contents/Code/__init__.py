...
def MainMenu():
	...
	# Add all programs
	oc.add(
	  DirectoryObject(key = Callback(TV4Shows, categoryName = "Alla program", 
	                                           categoryId = " "), 
		              title = "Alla program", 
		              thumb = None)
		  )
	...
	categories = JSON.ObjectFromURL(CATEGORIES_URL)
  	for category in categories:
		oc.add(
		  DirectoryObject(key = Callback(TV4Shows, categoryName = category["name"], 
...
def TV4Shows(categoryName, categoryId):
	...
	for program in programs:
		oc.add(
		  DirectoryObject(key = Callback(TV4ShowChoice, showName = program["name"], 
		                                                showId = program["id"], 
...
def TV4ShowChoice(showName, showId, art, thumb, summary):
	...
	if episodes['total_hits'] > 0 and clips['total_hits'] > 0: 
		oc.add(
		  	DirectoryObject(key = Callback(TV4Videos, showName = showName, 
		    	                                      showId = showId, 
		                                              art = art,
		                                              episodeReq = True), 
		                    title = "Hela program",
...
def TV4Videos(showName, showId, art, episodeReq, offset = 0):
	...
	if offset + ITEMS_PER_PAGE < videos['total_hits']:
		...
		oc.add(
		  	NextPageObject(key = Callback(TV4Videos, showName = showName, 
		    	                                     showId = showId, 
		       		                                 art = art,
